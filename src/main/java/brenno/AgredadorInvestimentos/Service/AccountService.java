...
import brenno.AgredadorInvestimentos.Client.BrapiClient;
...
public class AccountService {
    ...
    private double getTotal(Integer quantity, String stockId) {
        ...
        if (response.results() == null || response.results().isEmpty()) {
            throw new IllegalStateException("Nenhum resultado encontrado para o ativo: " + stockId);
        }

        var firstResult = response.results().get(0);
        var price = firstResult.regularMarketPrice();
